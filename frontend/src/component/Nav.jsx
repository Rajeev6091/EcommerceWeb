...
function Nav() {
  const { userData, setUserData } = useContext(userDataContext); // 🔥 get setUserData
  const { serverUrl } = useContext(authDataContext);
  ...
  return (
    <header className="w-full fixed top-0 left-0 bg-blue-600 text-white flex items-center justify-between px-4 h-14 shadow-sm z-50">
      ...
      <nav className="hidden md:flex gap-6 text-sm">
        ...
        <button onClick={() => navigate("/collections")} className="hover:underline">
          Collections
        </button>
        ...
        <div className="relative flex items-center gap-2">
          {showSearch && (
            <input
              type="text"
              placeholder="Search Here"
              className="w-48 md:w-60 bg-white text-black rounded px-3 py-1 shadow"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          )}
